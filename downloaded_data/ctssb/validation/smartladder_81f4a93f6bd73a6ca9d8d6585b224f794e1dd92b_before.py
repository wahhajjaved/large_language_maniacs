...
class Common(object):
    ...
    def set_appids(self,appids):
        self.CONFIG.set('gae','appid','|'.join(appids))
        with open(self.CONFIG_FILENAME, 'w') as configfile:
            self.CONFIG.write(configfile)

    def reload_appids(self):
        self.CONFIG.read(self.CONFIG_FILENAME)
        self.GAE_APPIDS = re.findall('[\w\-\.]+', self.CONFIG.get('gae', 'appid').replace('.appspot.com', ''))
    ...
    if ctypes and os.name == 'nt':
        ...
        if softwares:
            ...
            if softwares:
                ...
                #sys.exit(0)
    logging.info('getting appids from goagent plus common appid pool!')
    common.set_appids(get_appids())
    common.reload_appids()
    if common.PAC_ENABLE:
