...
def sendfile(request, filename, attachment=False, attachment_filename=None, mimetype=None, encoding=None):
    ...
    if attachment:
        ...
        if isinstance(unicode, attachment_filename):
            attachment_filename = attachment_filename.encode('utf-8')
        response['Content-Disposition'] = 'inline; filename="%s"' % attachment_filename
